...
const Page = () => {
    ...
    return <main className="flex flex-col h-screen max-h-screen overflow-hidden">
        <header className="border-b border-zinc-800 p-4 flex items-center justify-between bg-zinc-900/30">
            ...
            <button className="text-xs bg-zinc-800 hover:bg-red-600 px-3 py-1.5 rounded text-zinc-400 hover:text-white transition-all group flex items-center gap-2 disabled:opacity-50 cursor-pointer" onClick={() => destroyRoom()}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin">
            {messages?.messages.length === 0 && (
                <div className="flex items-center justify-center h-full">
                    <p className="text-zinc-600 text-sm font-mono">No messages yet, start a conversation</p>
                </div>
            )}
            ...
            <div className="p-4 border-t border-zinc-800 bg-zinc-900/30">
                <div className="flex gap-4">
                    ...
                    <button className="bg-zinc-800 text-zinc-400 px-6 text-sm font-bold hover:text-zinc-200 transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer" onClick={() => {
                        sendMessage({ text: input })
                        inputRef.current?.focus();
                    }}
